--
-- This search uses a bucketed priority queue. Each priority maintains a list
-- of nodes.
--
import System.Environment (getArgs)
import Data.List
import qualified Data.HashSet as HS
import qualified Data.BPQueueMax as PS
...
findNode :: Node -> HS.HashSet Element -> PS.BPQueueMax Node -> [Node]
findNode end visited pq =
    let que = PS.maxView pq
    in  case que of
        Nothing      -> []
        Just (x, xs) -> go x xs
  where go :: Node -> PS.BPQueueMax Node -> [Node]
        go Nil _ = []
        go x@(Node _ v) xs
