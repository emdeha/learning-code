...
class VertexDFS:
    ...
    def __repr__(self):
        return ("\tName:  " + str(self.name) + "\n"
            ...
            "\tParents: " + str(map(lambda p: p.name, self.parents)) + "\n")
...
class Graph:
    ...
    def addEdge(self, v, children, addParents=False):
        self.vertices[v.name] = children
        if addParents:
            map(lambda ch: ch.parents.append(v), children)
